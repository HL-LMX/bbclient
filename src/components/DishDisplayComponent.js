...
const DishDisplayComponent = ({
  ...
  return (
    <div
      ...
      style={{
        display: 'grid',
        gridTemplateColumns: 'repeat(11, 1fr)', // 11 columns: 1 for trash/placeholder + 6 + 2 + 1 + 1 = 11
        marginBottom: '1em',
        gap: '0.8em',
      }}
    >
      {isPastDate ? (
        // Render an empty placeholder so the grid layout remains consistent
        <span style={{ gridColumn: 'span 1' }}></span>
      ) : (
        // Render the trash button if the dish is unlocked
        <button
